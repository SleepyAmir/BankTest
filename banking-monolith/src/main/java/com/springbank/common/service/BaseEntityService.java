...
public abstract class BaseEntityService<T extends BaseEntity, ID extends Serializable, D> {
    ...
    public T updateEntity(@NonNull ID id, @NonNull Consumer<T> updateLogic) {
        log.debug("✏️ Updating {} entity with id: {}", getEntityTypeName(), id);
    ...
    public D updateFromDto(@NonNull ID id, @NonNull Consumer<D> updateLogic) {
        log.debug("✏️ Updating {} from DTO with id: {}", getEntityTypeName(), id);
    ...
    public boolean softDelete(@NonNull ID id) {
        log.info("🗑️ Soft deleting {} entity with id: {}", getEntityTypeName(), id);
        return repository.softDeleteWithRetry(id, 3);
    }

    @Transactional
    public boolean softDeleteWithRetry(@NonNull ID id, int maxRetries) {
        log.info("🗑️ Soft deleting {} entity with id: {}, maxRetries: {}", getEntityTypeName(), id, maxRetries);
        return repository.softDeleteWithRetry(id, maxRetries);
    }

    @Transactional
    public boolean softDeleteWithVersionCheck(@NonNull ID id) {
        log.info("🗑️ Soft deleting {} entity with version check, id: {}", getEntityTypeName(), id);
    ...
    public boolean restore(@NonNull ID id) {
        log.info("♻️ Restoring {} entity with id: {}", getEntityTypeName(), id);
    ...
    public void softDeleteBulk(@NonNull List<ID> ids) {
        log.info("🗑️ Soft deleting {} {} entities in bulk", ids.size(), getEntityTypeName());
    ...
    public void hardDelete(@NonNull ID id) {
        log.warn("⚠️ Hard deleting {} entity with id: {}", getEntityTypeName(), id);
        T entity = findEntityById(id);
        repository.delete(entity);
        log.warn("🗑️ {} entity hard deleted permanently", getEntityTypeName());
    }

    @Transactional
    public void hardDeleteBulk(@NonNull List<ID> ids) {
        log.warn("⚠️ Hard deleting {} {} entities in bulk", ids.size(), getEntityTypeName());
        ...
        log.warn("🗑️ {} entities hard deleted permanently", entities.size());
    }
